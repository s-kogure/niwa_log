...
**結論**: 現時点では、複数の信頼できる園芸情報源で継続的かつ実用上意味のある管理差を確認できる候補がないため、**セージ子分類は追加しない**。現状のセージ親1件（`aliases` にコモンセージを含む）で維持する。将来、パイナップルセージ・ホワイトセージ等について優先情報源（メーカー・種苗会社・NHK等）で具体的な栽培記事が確認できた場合に再検討する。
