...
初期 seed 対象: 親 17件（ラベンダー子分類4件・カモミール子分類2件を含み計23件）
※ 人間レビュー対応（2026-08-21）でチャイブ・チャービル・ステビアをDROP、レモンバーベナをADD。
　件数目安（20〜25件）は必達条件ではなく、無理な数合わせは行わない方針。
※ 追加人間レビュー対応（2026-09-03）で「セージ」を「コモンセージ」に改名（aliasesに旧名を保持）。
　チェリーセージは検討の結果 flower カテゴリの独立レコードとして追加（詳細は flower.md 参照）。

---

<!-- ここから植物ごとのセクションを追加する -->

進捗メモ（Session 1時点の調査履歴。以後のDROP/改名はセクション本文・上記総括行を正とする）：
- [x] アンカー4種（ローズマリー、イングリッシュラベンダー、バジル、ミント）
- [x] タイム、オレガノ、セージ（→コモンセージに改名。2026-09-03）
- [x] コリアンダー、パセリ、フェンネル、ディル
- [x] チャイブ（→2026-08-21 DROP）、レモングラス、レモンバーム
- [x] シソ
- [x] カモミール（ジャーマン）
- [x] ローマンカモミール（要2ソース確認）
...
## コモンセージ
※ 旧名「セージ」。2026-09-03の人間レビュー対応で改名（下記参照）。
...
| growth_form | shrub |
| parent_id | null |
| aliases | セージ, サルビア・オフィキナリス |
| preferred_moisture_level | 0.30 |
| moisture_band | 2 |
| is_moisture_anchor | false |
| watering_amount | light |
| watering_pace | 鉢土の表面が乾いたらたっぷり |
| watering_notes | 欧州原産で耐寒性は強いが、高温多湿の蒸れに弱い（耐暑性は弱め）。乾燥を好み、タイム・ローズマリーより若干湿潤を許容。地植えは根付き後水やりほぼ不要。チェリーセージ（flower、独立レコード）とは区別。 |
...
### 子分類要否の調査結論（人間レビュー対応・2026-08-21）

人間レビューで「セージは子分類を検討したほうがよい可能性がある」との指摘を受け、コモンセージ・パイナップルセージ・ホワイトセージについて子分類化の妥当性を調査した。

- **コモンセージ**: そもそも現行セージレコードの `aliases` に既に含まれており、一般名称「セージ」と学術的な代表種「コモンセージ」は同一種を指す。子分類ではなく統合済みの状態が適切。
- **パイナップルセージ**: `site:hyponex.co.jp` `site:kincho-engei.co.jp` を含む複数の検索で、専用の育て方記事が見つからなかった（一般的な「セージ」記事内で品種名として触れられる程度）。継続的な管理差を複数の信頼できる情報源で確認できなかったため、子分類の根拠を満たさない。また、パイナップルセージは赤い花を観賞する用途が強く、`flower` カテゴリの「サルビア」と役割が近接するため、herbのセージ子分類へ不用意に混在させるべきでないという人間レビューの指摘にも合致する。
- **ホワイトセージ**: 同様に hyponex・KINCHO園芸に専用記事が見つからず、「セージの一般的な育て方が参考になる」程度の情報にとどまった。乾燥地原産で本来は他のセージより乾燥志向が強いと想定されるが、複数の信頼できる情報源で確認できないため、推測で数値化しない（Issue #6の方針に従う）。

**結論（2026-08-21時点）**: 現時点では、複数の信頼できる園芸情報源で継続的かつ実用上意味のある管理差を確認できる候補がないため、**セージ子分類は追加しない**。現状のセージ親1件（`aliases` にコモンセージを含む）で維持する。将来、パイナップルセージ・ホワイトセージ等について優先情報源（メーカー・種苗会社・NHK等）で具体的な栽培記事が確認できた場合に再検討する。

### 名称変更・独立レコード化の再検討（追加人間レビュー対応・2026-09-03）

開発者本人がコモンセージ・チェリーセージを栽培しており、niwa.log上で別々に登録・管理したいとの人間レビューを受け、再調査した。

- **コモンセージ**（Salvia officinalis、欧州原産）: 耐寒性が強く、高温多湿の蒸れに弱い（耐暑性は弱め）。地植えは根付き後ほぼ水やり不要、鉢植えは表面が乾いたらたっぷり。
- **チェリーセージ**（主に Salvia microphylla、中南米原産）: 耐暑性が強く、耐寒性が弱い（霜で枯死のおそれ、寒冷地は鉢植えで冬は室内管理）。**コモンセージとは耐寒性・耐暑性が逆方向**という、niwa.logの栽培支援上重要な管理差が複数の信頼できる情報源で確認できた。低木状に木化し、開花期は5〜11月と長い。

2026-08-20時点の調査では専用記事が見つからなかったのはパイナップルセージ・ホワイトセージであり、チェリーセージは未調査だった。今回新たにハイポネックスの専用記事が見つかり、NHK「みんなの趣味の園芸」植物図鑑にも「サルビア（宿根性）」の代表品種としてコモンセージ・チェリーセージ双方の記載があることを確認した。
...
- コモンセージ
  1. https://www.hyponex.co.jp/garden_support/garden_support-218/ （ハイポネックス 園芸サポート）
  2. https://www.kincho-engei.co.jp/cultivation/detail/4228/ （KINCHO園芸）
  3. https://www.shuminoengei.jp/m-pc/a-page_p_detail/target_plant_code-286/target_tab-2 （NHK趣味の園芸 植物図鑑「サルビア（宿根性）」）
- チェリーセージ（詳細は `flower.md` 参照）
  1. https://www.hyponex.co.jp/plantia/22445/ （ハイポネックス Plantia、チェリーセージ専用記事）
  2. https://www.shuminoengei.jp/m-pc/a-page_p_detail/target_plant_code-286/target_tab-2 （NHK趣味の園芸 植物図鑑「サルビア（宿根性）」）

#### 既存flower「サルビア」との重複確認

既存の flower カテゴリ「サルビア」は Salvia splendens（サルビア・スプレンデンス、一年草の花壇苗）であり、NHKの分類上「宿根性サルビア」グループとは別種。コモンセージ・チェリーセージ（いずれも多年草・低木性）とは性質が異なり、直接の重複はない。

#### 結論（更新・人間レビュー最終判断）

「セージ」を「**コモンセージ**」に改名（`aliases` に旧名「セージ」を残す）。**チェリーセージ**は herb ではなく **flower カテゴリの独立レコード**として登録する（観賞価値が高く開花期も長いため、園芸上の用途・売り場が花ものに近いという人間レビュー判断）。両者・既存の flower「サルビア」の3レコードはいずれも親子関係を持たない独立レコード。チェリーセージの詳細な調査記録は `flower.md` に記載。パイナップルセージ・ホワイトセージは引き続き専用の栽培記事が確認できないため見送り。
...
> 人間レビューにより、MVP初期seedとしての優先度が相対的に低いと判断されDROPとなった。
...
| reference_url | https://www.shuminoengei.jp/m-pc/a-page_p_detail/target_plant_code-543/target_tab-2 |

> 人間レビュー対応（2026-08-21）でreference_urlを差し替え。従来のhorti.jp（GreenSnap系）から、
> 同じく参照URLとして確認済みだったNHK趣味の園芸へ変更（Issue #6優先情報源リストにより適合）。
> horti.jpは参照URL2として引き続き記録する。

### 参照URL
1. https://www.shuminoengei.jp/m-pc/a-page_p_detail/target_plant_code-543/target_tab-2 （NHK 趣味の園芸）
2. https://horti.jp/14550 （HORTI by GreenSnap）
...
> Issue #7 → 人間レビュー対応（2026-08-21）で除外。
> Issue #7時点でハーブ件数目安を満たす目的も兼ねて追加した経緯があり、家庭園芸での一般性が他ハーブより弱い。人間レビューでもMVP初期seedとしての優先度が低いと判断されDROPとなった。
> MVP初期seedからの除外はniwa.logでの利用価値・優先度による判断であり、将来のユーザーリクエスト等による追加を否定するものではない。
