...
-- 全 Session 完了。
--
-- 2026-08-21 人間レビュー反映: DROP21・ADD6・セージ子分類見送り（121→106件）
-- 2026-09-03 追加人間レビュー反映: セージ→コモンセージに改名、チェリーセージ追加
--   開発者本人が両種を栽培しており別々に登録・管理したいため再検討。
--   コモンセージ(herb)・チェリーセージ(flower)・サルビア(flower)を
--   それぞれ独立レコードとして扱う（耐寒性・耐暑性が逆方向、カテゴリも異なるため）。
--   106→107件（106件のうち「セージ」を「コモンセージ」に改名、チェリーセージを新規追加）。
--   詳細は docs/seed_research/herb.md・flower.md 参照。
--
-- 最終件数（2026-09-03、ローカルPostgreSQL 17で001〜010通し実行し検証済み）:
--   親99 + 子8 = 107件
--   category別内訳: herb23 orchid5 succulent9 fruit8 foliage16 flower28 vegetable18
...
insert into public.plants (
  ...
  ('コモンセージ', 'herb', 'shrub',      0.30, 'light',    '鉢土の表面が乾いたらたっぷり',
   '欧州原産で耐寒性は強いが、高温多湿の蒸れに弱い（耐暑性は弱め）。乾燥を好み、タイム・ローズマリーより若干湿潤を許容。地植えは根付き後水やりほぼ不要。チェリーセージ（flower）とは独立レコードで区別。',
   ARRAY['セージ', 'サルビア・オフィキナリス'], 'https://www.hyponex.co.jp/garden_support/garden_support-218/'),
  ...
  ('マジョラム',   'herb', 'herbaceous', 0.27, 'light', '表面が乾いてからたっぷり（過湿は根腐れの原因）',
   ...
   ARRAY['スイートマジョラム', 'マヨラナ'], 'https://www.shuminoengei.jp/m-pc/a-page_p_detail/target_plant_code-543/target_tab-2'),
...
insert into public.plants (
  ...
  ('金のなる木',           'succulent', 'other', 0.17, 'light',
   '春秋: 鉢土の表面が乾いたらたっぷり（夏は10日に1回夕方軽く）／冬: 2〜4週に1回程度',
   'ベンケイソウ科の丈夫な多肉植物。枯れる原因のほとんどは水のやりすぎで、乾かし気味の管理が基本。過湿による根腐れに注意。日当たりを好む。縁起物として家庭保有率が高い定番種。',
   ARRAY['クラッスラ', 'カネノナルキ'], 'https://www.hyponex.co.jp/garden_support/garden_support-238/'),
...
insert into public.plants (
  ...
  ('イチゴ',              'fruit', 'herbaceous', 0.58, 'moderate',
   '鉢: 表面が乾いたらたっぷり株元へ（午前中）／目安は秋〜春2〜5日に1回、夏は乾きやすくこまめに',
   '多年草。プランター栽培が一般的で根が浅く水切れに注意。乾燥を嫌うが過湿は根腐れの原因。葉・花に水をかけず株元へ与える。ランナーで子株を増やす。',
   ARRAY['いちご'], 'https://www.hyponex.co.jp/garden_support/garden_support-267/'),
...
insert into public.plants (
  ...
  ('ゴムノキ',              'foliage', 'tree', 0.50, 'moderate',
   '春秋: 鉢土の表面が乾いたらたっぷり／夏: 毎日＋葉水／冬: 乾かし気味＋こまめな葉水で加湿',
   '強健で育てやすい観葉植物の定番。過湿による根腐れに注意しつつ、夏は高温多湿を好み毎日の水やりが必要になることも。冬は室内の暖房による乾燥対策として葉水を推奨。',
   ARRAY['フィカス・エラスティカ', 'インドゴムノキ'], 'https://www.hyponex.co.jp/garden_support/garden_support-242/'),
...
insert into public.plants (
  ...
  -- 追加人間レビュー対応（2026-09-03）: 開発者本人が栽培しており独立登録を希望。
  -- サルビア（S. splendens、一年草の花壇苗）とは別種（S. microphylla系、多年草・低木）で
  -- 耐寒性・耐暑性の傾向も異なるため独立レコードとして扱う。herb「コモンセージ」とも独立。
  ('チェリーセージ',         'flower', 'shrub', 0.32, 'light',
   '鉢: 表面が乾いたらたっぷり（夏は日中を避け朝夕に）／地植え: 根付き後は基本不要',
   '中南米原産で耐暑性は強いが耐寒性は弱く、霜で枯死のおそれがある（寒冷地は鉢植えで冬は室内へ）。低木状に木化し、開花期は5〜11月と長い。同じセージ属のherb「コモンセージ」とは耐寒性・耐暑性が逆方向。',
   ARRAY['サルビア・ミクロフィラ'], 'https://www.hyponex.co.jp/plantia/22445/'),
  ...
  ('カレンデュラ',           'flower', 'herbaceous', 0.40, 'moderate',
   '鉢: 表面が乾いたら鉢底から流れるまで／地植え: 降雨任せで基本不要／冬は乾燥気味に管理',
   '一年草。丈夫で育てやすい秋まき春咲きの定番花壇苗。過湿は根腐れの原因。花に直接水をかけないよう株元へ与える。',
   ARRAY['キンセンカ'], 'https://www.hyponex.co.jp/plantia/plantia-15467/'),
  ...
  ('ハイビスカス',           'flower', 'shrub', 0.68, 'heavy',
   '生育期(春〜秋): 土の表面が乾いたら鉢底から流れるまで（夏は朝夕2回も）／冬: 乾かし気味に回数を減らす',
   '常緑低木。生育旺盛な夏は水を盛んに吸収し、水切れで葉先枯れ・蕾落ちが起きやすい。鉢植えは特に水切れ注意。冬は休眠気味になり乾燥気味の管理に切り替える。',
   ARRAY[]::text[], 'https://www.hyponex.co.jp/plantia/plantia-14499/')
...
insert into public.plants (
  ...
  ('ゴーヤ',              'vegetable', 'vine', 0.68, 'moderate',
   '土の表面が乾いたらたっぷり（朝の涼しい時間帯に）／真夏は朝夕2回も',
   'つる性でグリーンカーテンにも利用される夏野菜。生育旺盛で水切れに弱く、実つきが悪くなる。水は株元へ与え葉にはかけない。プランターは地植えより乾きやすくこまめに確認。',
   ARRAY['ニガウリ'], 'https://www.hyponex.co.jp/plantia/plantia-8656/'),
